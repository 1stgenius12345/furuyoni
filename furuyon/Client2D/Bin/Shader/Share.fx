...
//static float BlurOffset[25] = { 0.f, 0.1f, 0.2f,0.3f,0.4f};
//static float BlurOffset2[25] = { 0.f, 0.1f, 0.2f,0.3f,0.4f };
...
PS_OUTPUT_SINGLE FullScreenPS(VS_OUTPUT_NULLBUFFER input)
{
	...
	float value = g_PostProcessEmpty.x;
	if (g_PostProcessBlur == 1)
	{
		for (int i = 0; i < 49; ++i)
		{	
			BlurColor += g_BaseTexture.Sample(g_AnisotropicSmp, input.UV + float2(((i/7)-3)* 0.1f, ((i % 7) - 3) * 0.1f) * gaussian7x7[i] * value);
		
		}


		float temp = 1.f + g_PostProcessEmpty.y;
