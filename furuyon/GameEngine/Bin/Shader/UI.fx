...
VS_OUTPUT2_UI UILogVS(VS_INPUT2_UI input)
{
	VS_OUTPUT2_UI	output = (VS_OUTPUT2_UI)0;
	float4  clip = float4(0.f, 1.f, 0.f, 0.81f);
	float4  clip1 = float4(0.f, -1.f, 0.f, 0.64f);



	output.Pos = mul(float4(input.Pos, 1.f), g_matUIWVP);
	output.UV = ComputeUIAnimation2DUV(input.UV);
	output.clip = dot(mul(float4(input.Pos, 1.f), g_matUIWVP), clip);
	output.clip1 = dot(mul(float4(input.Pos, 1.f), g_matUIWVP), clip1);
	return output;
}

PS_OUTPUT_SINGLE UILogPS(VS_OUTPUT2_UI input)
{
	PS_OUTPUT_SINGLE output = (PS_OUTPUT_SINGLE)0;
