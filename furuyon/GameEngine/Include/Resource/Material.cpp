...
float CMaterial::GetEmpty() const
{
	return m_empty;
}

void CMaterial::AddEmpty(float f)
{
	m_empty += f;
	m_pCBuffer->SetEmpty(m_empty);
}

void CMaterial::SetEmpty(float f)
{
	m_empty = f;
	m_pCBuffer->SetEmpty(m_empty);
}
...
void CMaterial::AddBaseColor(float r, float g, float b, float a)
{
...
void CMaterial::AddEmissiveColor(const Vector4& Color)
{
	m_EmissiveColor += Color;
