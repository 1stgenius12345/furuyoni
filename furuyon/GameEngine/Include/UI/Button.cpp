...
void CButton::Update(float DeltaTime)
{
	...
	if ((GetAsyncKeyState(VK_LBUTTON) & 0x8000) && WndActive)
	{
		if (!m_Down && !m_Push)
			m_Down = true;
...
bool CButton::SetStateTexture(const std::string& Name, const TCHAR* FileName, const std::string& PathName)
{
	if (!m_Scene->GetResource()->LoadTexture(Name, FileName, PathName))
		return false;

	m_ButtonState[(int)Button_State::Normal].Texture =
		m_Scene->GetResource()->FindTexture(Name);
	m_ButtonState[(int)Button_State::MouseOn].Texture =
		m_Scene->GetResource()->FindTexture(Name);
	m_ButtonState[(int)Button_State::Click].Texture =
		m_Scene->GetResource()->FindTexture(Name);
	return false;
}
