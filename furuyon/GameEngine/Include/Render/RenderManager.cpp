...
void CRenderManager::SetEmpty(Vector2 x)
{
	m_PostProcessCBuffer->SetEmpty(x);
}

void CRenderManager::AddEmpty(Vector2 x)
{
	m_PostProcessCBuffer->AddEmpty(x);
}
...
void CRenderManager::Render2D(float DeltaTime)
{
	...
	std::sort(m_pRenderList2D[RT2D_BACKMAP], m_pRenderList2D[RT2D_BACKMAP] + m_RenderList2DCount[RT2D_BACKMAP],
		CRenderManager::Sort2DObject);

	m_DepthDisable->SetState();
	for (int j = 0; j < m_RenderList2DCount[RT2D_BACKMAP]; ++j)
	{
		m_pRenderList2D[RT2D_BACKMAP][j]->Render(DeltaTime);
	}
