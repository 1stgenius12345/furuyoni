...
CInput::CInput() :
	...
	m_MousePut(false),
	m_MouseClick(false),
	m_MouseRightPut(false),
	m_MouseRightPush(false),
	m_MouseRightPushTime(0.f)
{
...
void CInput::UpdateMouse(float DeltaTime)
{
	...
	bool WndActive = CEngine::GetInst()->GetActiv();

	if (m_MouseState.rgbButtons[0]&& WndActive)
	{
		m_MousePush = true;
		m_MousePushTime = 0.f;
		m_MousePut = false;
	...
	if (m_MouseState.rgbButtons[1] && WndActive)
	{
		m_MouseRightPut = true;
		m_MouseRightPush = true;
		m_MouseRightPushTime = 0.f;
	}


	if(m_MousePush)
		m_MousePushTime += DeltaTime;
	if (m_MouseRightPush)
		m_MouseRightPushTime += DeltaTime;
	...
	if (m_MouseRightPut)
		m_MouseRightPut = false;
	if (m_MouseRightPushTime > 0.02f)
	{
		m_MouseRightPushTime -= 0.02f;
		m_MouseRightPush = false;
	
		m_MouseRightPut = true;
	}
	if (m_MousePushTime > 0.02f)
	{
		m_MousePushTime -= 0.02f;
		m_MousePush = false;
		m_MousePut = true;
